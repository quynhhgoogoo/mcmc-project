...
X = unifrnd(0, 5, n);  
y = normrnd(beta0 + beta1*X, 1/sqrt(tau), n); % generate some synthetic data

figure;  % create a blank figure
plot(X, y, 'o','MarkerFaceColor', 'b', 'MarkerSize',4); % plot X vs y
xlabel ('X')
ylabel ('y')
...
proposal_PDF_mala = @(beta_proposed, beta_current, mu, Ssigma) normpdf(); % TODO

sample_from_proposal_PDF_mala = @(beta_current, mu, Ssigma) normrnd(); % TODO
